...
cllc_melt <- cllc_melt[,attack:=list(list(cllc_category[[1]][["attack speed"]])),by=1:nrow(cllc_melt)]
attack <- cllc_melt[,unlist(attack),by=.(value)]
attack %>%  setnames(c("V1","value"),c("attack_speed","value"))
attack[,stars:=1:.N-1,by=value]

cllc_melt <- cllc_melt[,movement:=list(list(cllc_category[[1]][["movement speed"]])),by=1:nrow(cllc_melt)]
...
cllc_melt <- cllc_melt[,health:=list(list(cllc_category[[1]][["health"]])),by=1:nrow(cllc_melt)]
health <- cllc_melt[,unlist(health),by=.(value)]
health %>%  setnames(c("V1","value"),c("health","value"))
health[,stars:=1:.N-1,by=value]

cllc_melt <- cllc_melt[,damage:=list(list(cllc_category[[1]][["damage"]])),by=1:nrow(cllc_melt)]
