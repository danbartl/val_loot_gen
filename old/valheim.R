...
creatures2 <- creatures2a[!is.na(hp) & !is.na(damage) & type_behavior!="Passive"] 
setkeyv(creatures2,"name")

creatures2["Eikthyr",damage:=40]
creatures2["The Elder",damage:=80]
creatures2["Bonemass",damage:=100]
creatures2["Moder",damage:=130]
creatures2["Yagluth",damage:=150]


creatures2[,movement_speed:=1]
creatures2[name=="Deathsquito",movement_speed:=3]
creatures2[name=="Wolf",movement_speed:=2]

creatures2[,accuracy:=1]
creatures2[type_behavior=="Boss",accuracy:=3]
creatures2[name %in% c("Moder","Yagluth"),accuracy:=5]

creatures2[name=="Deathsquito",accuracy:=0.1]
creatures2[,effective_hp:=hp/accuracy]

creatures2[,effective_damage:=0.5*damage^2/(hero_armor + 0.5 * damage)*movement_speed]
creatures2[,hits_to_kill:=hero_hp/effective_damage]
creatures2[,danger:=hero_hp/effective_damage]

damage = 750
0.5*damage^2/(135 + 0.5 * damage)

setorderv(creatures2,"damage")

creatures2[,hp_scaled:=(10+pmin(hp,200))^0.75]
creatures2[hp>200,hp_scaled:=hp_scaled+(10+hp-200)^0.5]
creatures2[,danger:=1/hits_to_kill*hp_scaled]
